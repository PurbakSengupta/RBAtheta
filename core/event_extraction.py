...
def significant_events(data, threshold):
    ...
    for i in range(length):
        ...
        new_event = [int(start), int(i + 1), data[int(start)], data[int(i + 1)]]
        events.append(new_event)
        prev = data[0]  
        start = i + 1  
    delta = []
    ...
    for event in m_events:
        new_row = pd.Series([event[0],
                            ...
                            index=__significant_events.columns)
        __significant_events = pd.concat([__significant_events, new_row.to_frame().T], ignore_index=True)

    lambdas = lam(__significant_events, threshold)
    __significant_events = pd.concat([__significant_events, lambdas], axis=1)
    print("Significant Events DataFrame:")
    print(__significant_events.head())
    print(f"Detected Events: {events_bigger_than_threshold}")  # Debug statement
    if not events_bigger_than_threshold:
...
def stationary_events(data, threshold):
    ...
    while start < length:
        ...
        if forward > 0:
            new_event = pd.Series([start,
                                   start + forward,
                                   forward,
                                   sum(data[start: (start + forward + 1)]) / (forward + 1)],
                                  index=__stationary_events.columns)
            __stationary_events = pd.concat([__stationary_events, new_event.to_frame().T], ignore_index=True)

        start += forward + 1

    lambdas = lam(__stationary_events, threshold)
    __stationary_events = pd.concat([__stationary_events, lambdas], axis=1)
    print("Stationary Events DataFrame:")
    print(__stationary_events.head())
